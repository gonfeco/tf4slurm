...
def DistributedTrain(FLAGS, task_index, TFCluster, TFServer,QueueHook):
	...
	with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" %task_index,cluster=TFCluster)):
		...
		with tf.name_scope('adam_optimizer'):
			...
			SyncOtimizer=tf.train.SyncReplicasOptimizer(Optimizer,
				replicas_to_aggregate=FLAGS.workers,
				total_num_replicas=FLAGS.workers,
				use_locking=True,
				name="sync_replicas"
			)
			train_step=SyncOtimizer.minimize(cross_entropy,
	...
	sync_replicas_hook=SyncOtimizer.make_session_run_hook(is_chief,num_tokens=0)
	...
	if is_chief:
		...
		hooks.append(HOOKS.NewSummarySaverHook(MergedSummaryOperation, FLAGS.model_dir+"/Training", SummarySteps,
			FLAGS.Iterations, features=x, labels=y_, dropout=keep_prob, batchx=TrainingImagesValidation,
			batchy=TrainingLabelsValidation, dropout_value=1.0)
		)
